...
namespace SVNMonitor.Helpers
{
	internal class KeyboardHookHelper
	{
		...
		private static void CreateKeyActionsMap()
		{
			...
			AddKeyAction(ApplicationSettingsManager.Settings.KeyboardShortcutShowMainWindow, MainForm.FormInstance.ShowOrHideForm);
			AddKeyAction(ApplicationSettingsManager.Settings.KeyboardShortcutUpdateAllAvailable, Source.SVNUpdateAllAvailable);
			AddKeyAction(ApplicationSettingsManager.Settings.KeyboardShortcutCheckModifications, Source.SVNCheckAllModifications);
			AddKeyAction(ApplicationSettingsManager.Settings.KeyboardShortcutCheckSources, Updater.Instance.QueueUpdates);
		}
		...
		private static void CreateKeyboardHook()
		{
			try
			{
				...
				KeyHook.KeyPressed += KeyboardHookHelper.KeyHook_KeyPressed;
			}
			catch (Exception ex)
			{
		...
		private static void RegisterKeys()
		{
			foreach (KeyInfo keyInfo in keyActions.Keys)
			{
				...
				{
					ErrorHandler.Append(Strings.ErrorAssignShortcutKey_FORMAT.FormatWith(new object[]
					{
						keyInfo
					}), keyInfo, ex);
				}
