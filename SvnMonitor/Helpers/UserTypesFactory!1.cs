...
namespace SVNMonitor.Helpers
{
	internal class UserTypesFactory<T>
	{
		...
		public static List<UserTypeInfo> GetAvailableUserTypes(TypeRequirements requirements)
		{
			...
			string[] moreAssemblies = new[]
			{
				FileSystemHelper.AppData, Path.Combine(FileSystemHelper.AppData, "Addins"), Application.StartupPath, Path.Combine(Application.StartupPath, "Addins")
			};
		...
		public static string GetUserTypeDisplayName(System.Type userType)
		{
			...
			if (attribute != null)
			{
				...
				object resourceTextObject = resourceManagerObject.GetType().InvokeMember("GetString", BindingFlags.InvokeMethod, null, resourceManagerObject, new object[]
				{
