...
public class RecommendWizard : BaseDialog
{
	...
	private void InitializeComponent()
	{
		...
		this.btnRecommend.add_Click(new EventHandler(this.btnRecommend_Click));
		...
		this.btnCommit.add_Click(new EventHandler(this.btnCommit_Click));
	...
	{
		if (this.recommended && !this.committed)
		{
			DialogResult result = MessageBox.Show(this, Strings.AskCloseWizardWithoutCommitting, Strings.SVNMonitorCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2);
	...
	public static DialogResult Recommend(Source source, long revision)
	{
		RecommendWizard wizard = new RecommendWizard();
		wizard.Source = source;
		wizard.Revision = revision;
		wizard.CommitMessage = ApplicationSettingsManager.Settings.RecommendationMessage.Replace("${REVISION}", revision.ToString());
	...
	private void RefreshButtons()
	{
		...
		this.btnRecommend.Enabled = (this.recommended ? 0 : this.updated);
	...
	{
		long num = 0L;
		base.Text = string.Format("Recommend revision {0} of '{1}'", this.revision, this.source.Name);
		string revisionString = (this.Revision == (long)0 ? "{0}" : num.ToString());
		string sourceName = (!this.Source ? "{1}" : this.Source.Name);
