...
namespace SVNMonitor.Actions
{
	...
	internal class InfoBalloonTipAction : BalloonTipAction
	{
		...
		protected override void Run(List<SVNLogEntry> logEntries, List<SVNPath> paths)
		{
			...
			{
				CreateTipTextForSingleLogEntry(logEntries[0]);
			}
			else if ((paths != null) && (paths.Count == 1))
			{
				CreateTipTextForSinglePath(paths[0]);
			}
			else
			{
				CreateTipTextForMultipleUpdates(logEntries, paths);
			}
		...
		{
			get { return base.ShowBalloonTip; }
			set { base.ShowBalloonTip = value; }
		}

		public override string SummaryInfo
		{
			get { return "Show a balloon tip with a list of sources that have available updates."; }
		}
		...
		{
			[DebuggerNonUserCode]
			get { return base.TipIcon; }
			[DebuggerNonUserCode]
			set { base.TipIcon = value; }
		}
		...
		{
			[DebuggerNonUserCode]
			get { return base.TipText; }
			[DebuggerNonUserCode]
			set { base.TipText = value; }
		}
		...
		{
			[DebuggerNonUserCode]
			get { return base.TipTitle; }
			[DebuggerNonUserCode]
			set { base.TipTitle = value; }
		}
	}
}